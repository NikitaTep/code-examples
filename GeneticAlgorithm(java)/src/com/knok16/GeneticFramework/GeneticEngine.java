...
public class GeneticEngine extends Thread{
	...
	public long[][] genomListOffsprings;
	private long[] actual;
	private long[] fitnessFunctionResult;
	private long currentGeneration = 0;

	private int maxResult;
	private int resFound = 0;
	public long[] bestGenom;
	public long[] missioner;		//genom for island model
	public  long[][] inputGenomList;
	...
	public void run() {
		...
		if (this.inputGenomList == null)
		//Generate 1st generation
			this.generateFirstGeneration();
		else
			this.genomListParents = this.inputGenomList;
		
		while (this.currentGeneration < this.generationCount && resFound == 0) {
		...
		int bestFitnessFunctionResult = 0;
		for (long[] genom : this.genomListOffsprings) {
			int fitnessFunctionResult = this.fitnessFunction.run(genom);
			if (fitnessFunctionResult > bestFitnessFunctionResult) {
		//		bestGenom = genom;
				missioner = genom;
				bestFitnessFunctionResult = fitnessFunctionResult;
			}
		}
		//System.out.println("\nBest Result: \t"+bestFitnessFunctionResult);
		//System.out.println("\nCount generation: \t"+this.currentGeneration);
	}
	...
	private long getFitnessFunctionResult(int genomNumber) {
		if (this.actual[genomNumber] != this.currentGeneration) {
			...
			if (this.fitnessFunctionResult[genomNumber] >= this.maxResult){
				this.resFound = 1;
				this.bestGenom = this.genomListParents[genomNumber];
				System.out.println("\nI FOUND! Count generation: \t"+this.currentGeneration);
			}
	...
	public  void  setInputGenomList(long[][] inputGenomList){
		this.inputGenomList = inputGenomList;
	}
}